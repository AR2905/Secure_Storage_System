...
class WordEncryptor:
    ...
    def encrypt_word_with_aes(self, password):
        ...
        encrypted_word_data_aes = encryptor.update(self.pad_word(word_data)) + encryptor.finalize()
        self.encrypted_file_combined = salt_aes + iv_aes + encrypted_word_data_aes
    ...
    def encrypt_word_with_3des(self, password):
        ...
        encryptor = cipher.encryptor()
        encrypted_word_data_3des = encryptor.update(self.pad_word(self.encrypted_file_combined)) + encryptor.finalize()
        
        self.encrypted_file_combined = salt_3des + iv_3des + encrypted_word_data_3des
        return self.encrypted_file_combined

    def run(self):
        self.encrypt_word_with_aes(self.aes_key_hex)
        self.enced_Data = self.encrypt_word_with_3des(self.des3_key_hex)
...
if __name__ == "__main":
    input_file = 'YOUR_WORD.docx'  # Provide the path to your Word document
    word_encryptor = WordEncryptor(input_file)
    word_encryptor.run()
    word_encryptor.w_file()
    
    aes_key = word_encryptor.aes_key_hex
    des3_key = word_encryptor.des3_key_hex
